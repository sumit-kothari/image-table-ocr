...
def download_image_to_tempdir_fastAPI(fileobject, filename=None):
    if filename is None:
        filename = ''.join(random.SystemRandom().choice(string.ascii_letters) for _ in range(20)) + ".png"
...
def main(url):
    ...
    image_filepath = download_image_to_tempdir_fastAPI(url)
